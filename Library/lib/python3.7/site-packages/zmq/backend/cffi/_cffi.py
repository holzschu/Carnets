...
# load zmq.h file instead:
with open(os.path.join(here, 'zmq.h')) as f:
    _zmqh = f.read()
ffi.cdef(_zmqh)
# pre-processed standard headers:
with open(os.path.join(here, 'preprocessed.h')) as f:
    _preprocessed = f.read()
ffi.cdef(_preprocessed)
# pre-processed standard defines:
with open(os.path.join(here, 'defines.h')) as f:
    _defines = f.read()
ffi.cdef(_defines)
